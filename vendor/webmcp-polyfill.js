// Vendored 2026-09-18 from https://github.com/GoogleChromeLabs/webmcp-tools
// (demos/shared/webmcp-polyfill.js). Re-sync upstream when it changes.
/**
...
(function () {
  ...
  class ModelContext extends EventTarget {
    ...
    async getTools(options = {}) {
      ...
      for (const tools of remoteToolsResults) {
        allTools.push(...tools);
      }
  ...
  function polyfillCSSPseudoClasses(win) {
    ...
    const processCSSText = (text) => {
      ...
      const newText = text.replace(regexForm, '.tool-form-active').replace(regexSubmit, '.tool-submit-active');
